...
class strataMesh():
    ...
    def move_mesh(self, dispX, dispY, cumdiff, verbose=False):
        ...
        if verbose:
            print(" - move stratal mesh ", time.time() - walltime)
        ...
        if verbose:
            print(" - create deformed stratal mesh arrays ", time.time() - walltime)

        # Build the kd-tree
        walltime = time.time()
        deformtree = cKDTree(deformXY)
        if verbose:
            print(" - create deformed stratal mesh kd-tree ", time.time() - walltime)

        walltime = time.time()
        distances, indices = deformtree.query(self.xyi, k=4)
        if verbose:
            print(" - query stratal mesh kd-tree ", time.time() - walltime)
        ...
        if verbose:
            print(" - perform stratal mesh interpolation ", time.time() - walltime)

        if verbose:
            print(" - moving stratal mesh function ", time.time() - st_time)
