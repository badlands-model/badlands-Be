...
def construct_mesh(input, filename, verbose=False):
    ...
    * define the partitioning when parallelisation is enable.
    ...
    force = forceSim.forceSim(input.seafile, input.seapos, input.rainMap,
                ...
                input.carbTime,input.icecurve,input.ice_map,input.Qmap,input.tEnd)
    ...
    if verbose:
        print(" - FV mesh ", time.time() - walltime)
...
def reconstruct_mesh(recGrid, input, verbose=False):
    ...
    * redefine the partitioning when parallelisation is enable.
    ...
    if verbose:
        print(" - partition TIN amongst processors ", time.time() - walltime)
    ...
    if verbose:
        print(" - reconstructed FV mesh ", time.time() - walltime)
...
def _define_TINparams(totPts, input, FVmesh, recGrid, verbose=False):
    ...
    if verbose:
        print(" - define paramters on TIN grid ", time.time() - walltime)
...
def _build_strateroMesh(input, FVmesh, recGrid, cumdiff, verbose=False):
    ...
    if input.laytime > 0:
        ...
        if verbose:
            print(" - create stratigraphic regions ", time.time() - walltime)
    ...
    if input.erolays and input.erolays >= 0:
        ...
        if verbose:
            print(" - create erodibility mesh ", time.time() - walltime)
...
def _init_wavesed(input, z0,recGrid, force, verbose=False):
    ...
    if verbose:
        print("   - Initialise wavesed grid ", time.time() - walltime)
