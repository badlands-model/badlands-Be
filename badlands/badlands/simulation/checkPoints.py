...
def write_checkpoints(input, recGrid, lGIDs, inIDs, tNow, FVmesh, \
                      ...
                      mapero=None, cumflex=None,Top_Shield=None,Bed_Be=None,\
                      Qprop=None,Prate=None,Thck=None):
    """
    ...
    Args:
        ...
        Top_Shield: topographic shielding (for cosmogenic nuclides production) (CP)
        Bed_Be: 10Be concentration (CP)
        Qprop: Quartz proportion (CP)
        Thck: Ice Thickness (CP)
    ...
    if input.flexure:
        visualiseTIN.write_hdf5_flexure(input.outDir, input.th5file, step, tMesh.node_coords[:,:2],
                                    ...
                                    cumhill[lGIDs], cumfail[lGIDs], cumflex[lGIDs], Top_Shield[lGIDs], Bed_Be[lGIDs],
                                    Qprop[lGIDs],Prate[lGIDs],Thck[lGIDs],FVmesh.outCells, input.oroRain,
                                    eroOn, flow.erodibility[lGIDs], FVmesh.control_volumes[lGIDs],
                                    waveOn, BeOn, IceOn, meanH, meanS, wdiff, rockOn, prop[lGIDs,:], force.sealevel)
    else:
        visualiseTIN.write_hdf5(input.outDir, input.th5file, step, tMesh.node_coords[:,:2],
                                elevation[lGIDs], rain[lGIDs], visdis[lGIDs], cumdiff[lGIDs],
                                cumhill[lGIDs], cumfail[lGIDs], Top_Shield[lGIDs], Bed_Be[lGIDs],
                                Qprop[lGIDs],Prate[lGIDs],Thck[lGIDs], FVmesh.outCells, input.oroRain, eroOn,
                                flow.erodibility[lGIDs], FVmesh.control_volumes[lGIDs],
                                waveOn, BeOn, IceOn, meanH, meanS, wdiff, rockOn,
    ...
    visualiseTIN.write_xmf(input.outDir, input.txmffile, input.txdmffile, step, tNow, tcells,
                           ...
                           input.oroRain, eroOn, waveOn, rockOn, BeOn, IceOn, prop.shape[1])

    visualiseFlow.write_xmf(input.outDir, input.fxmffile, input.fxdmffile, step, tNow,
                            fline, fnodes, input.fh5file)

    print("   - Writing outputs (%0.02f seconds; tNow = %s)" % (time.time() - out_time, tNow))
