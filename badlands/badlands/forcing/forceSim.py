...
class forceSim:
    ...
    Args:
        ...
        ice_map: csv map with the maximum glacier extent (CP 2020)
        tempcurve: EPICA temperature csv curve for ice evolution (CP 2020)
        Qmap: csv map with initial quartz concentration in rocks (CP 2020)
    ...
    def __init__(self, seafile = None, sea0 = 0., MapRain = None, TimeRain = None, ValRain = None,
                 ...
                 carbValSp1 = None, carbValSp2 = None, TimeCarb = None,icecurve = None,ice_map=None,Qmap=None,tEnd=0.):
    ...
    def _build_Ice_function(self):
        """
        Using Pandas library to read the ice ratio file and define interpolation
        function based on Scipy 1D linear function.
        """
        tempdata = pandas.read_csv(self.icecurve,sep=r'\s+',header=None,dtype=numpy.float)
        self.temptime = tempdata.values[:,0] 
        self.tempval  = tempdata.values[:,1]
        self.IceFunc  = interpolate.interp1d(self.temptime,self.tempval,kind='linear')
    ...
    def get_Ice(self,inIDs):
        ...
        tinI = interpolate.interpn((self.regX,self.regY), rectI,self.tXY[inIDs,:],method = 'linear')
        return tinI
