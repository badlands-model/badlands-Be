...
class xmlParser:
    ...
    def _get_XmL_Data(self):
        ...
        if spl is not None:
            ...
            if element is not None:
                self.a_rat = float(element.text)
            element = None
            element = spl.find('b_rating')
            if element is not None:
                self.b_rat = float(element.text)
            element = None
        ...
        # Extract cosmonuclide production parameters (CP2020)
        cosmo = None
        cosmo = root.find('cosmonuclides')
        if cosmo is not None:
            element = None
            element = cosmo.find('quartz_map')
            if element is not None:
                self.Qmap = element.text
                if not os.path.isfile(self.Qmap):
                    raise ValueError ('Quartz map file not found ')
                element = None
                element = cosmo.find('denudation_rate')
                if element is not None:
                    self.initdenrate=float(element.text)
                element = None
                element = cosmo.find('latitude')
                if element is not None:
                    self.latitude = float(element.text)
                element = None
                element = cosmo.find('shielding_factor')
                if element is not None:
                    self.shieldfact = float(element.text)

        # Extract glacier coverage parameters (CP2020)
        glacier = None
        glacier = root.find('ice_cover')
        if glacier is not None:
            self.is_ice = 1.
            element = None
            element = glacier.find('ice_curve')
            print('loading ice curve')
            if element is not None:
                self.icecurve = element.text
                if not os.path.isfile(self.icecurve):
                    raise ValueError ('Ice curve file not found ')
            element = None
            element = glacier.find('ice_map')
            print('loading ice map')
            if element is not None:
                self.ice_map = element.text
                if not os.path.isfile(self.ice_map):
                    raise ValueError ('Ice cover file not found ')
            element = None
            element = glacier.find('ice_erosion')
            if element is not None:
                self.ice_diff = float(element.text)
