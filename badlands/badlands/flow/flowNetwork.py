...
class flowNetwork:
    ...
    def compute_hillslope_diffusion(self, elev, qpr, nbe, neighbours, edges, distances, globalIDs, type, Sc):
        """
        ...
        Returns:
            ...
            - diff_Qz - numpy array containing Quartz proportion in sediments deposited by hillslope processes.
            - diff_Be - numpy array containing 10Be concentration in sediments deposited by hillslope processes.

        """
        numPts = len(elev)
        diff_Qz = numpy.zeros(numPts)
        diff_Be = numpy.zeros(numPts)

        if type == 0:
            if Sc > 0.:
                tSc = numpy.zeros(1)
                tSc[0] = Sc
                diff_flux,diff_Qz,diff_Be = sfd.diffusionnl(tSc, elev, qpr, nbe, self.borders2, neighbours, edges, distances, globalIDs)
            else:
                diff_flux,diff_Qz,diff_Be = sfd.diffusion(elev, qpr, nbe, self.borders2, neighbours, edges, distances, globalIDs)
        else:
            diff_flux = sfd.diffusionero(elev, self.borders2, neighbours, edges, distances, globalIDs)

        return diff_flux, diff_Qz, diff_Be

    def compute_marine_diffusion(self, elev, qpr, nbe, depoH, neighbours, edges, distances, coeff,
        ...
        diff_Qz
            numpy array containing Quartz proportion in sediments deposited by marine diffusion processes.
        diff_Be
            numpy array containing 10Be concentration in sediments deposited by marine diffusion processes.
        """

        diff_flux, diff_Qz, diff_Be, ndt = flowalgo.diffmarine(elev, qpr, nbe, self.borders, depoH, neighbours, edges,
                                           distances, coeff, globalIDs, seal, maxth, tstep)

        # Send local diffusion flux globally
        mindt = numpy.array(ndt)

        return diff_flux, diff_Qz, diff_Be, mindt

    def compute_failure_diffusion(self, elev, qpr, nbe, depoH, neighbours, edges, distances, coeff,
        ...
        diff_Qz
            numpy array containing Quartz proportion in sediments deposited by slope failure processes.
        diff_Be
            numpy array containing 10Be concentration in sediments deposited by slope failure processes.
        mindt
            maximum time step (in years) to ensure stable results

        """
        diff_flux, diff_Qz, diff_Be, ndt = flowalgo.difffailure(elev, qpr, nbe, self.borders, depoH, neighbours, edges,
    ...
    def compute_sediment_marine(self, elev, qpr, nbe, dep, sdep, coeff, neighbours, seal, maxth,
        ...
        diff_Qz
            numpy array containing Quartz content in sediments deposited by marine processes.
        diff_Be
            numpy array containing 10Be concentration in sediments deposited by marine processes.
        """
        diff_prop, diff_flux, diff_Qz, diff_Be = flowalgo.diffsedmarine(elev, qpr, nbe, self.borders, dep, sdep,
                                           seal, maxth, coeff, neighbours, edges, distances, globalIDs)

        return diff_prop, diff_flux, diff_Qz, diff_Be

    def compute_sediment_hillslope(self, elev, qpr, nbe, difflay, coeff, neighbours,
        ...
        diff_Qz
            numpy array containing Quartz content in sediments deposited by hillslope processes.
        diff_Be
            numpy array containing 10Be concentration in sediments deposited by hillslope processes.
        """
        sumdiff, diff_Qz, diff_Be, ero, depo = flowalgo.diffsedhillslope(elev, qpr, nbe, self.borders, difflay,
                                           layh, coeff, neighbours, edges, distances, globalIDs)

        return sumdiff, diff_Qz, diff_Be, ero, depo
    ...
    def compute_flow(self, sealevel, elev, Acell, rain, melt_waters=None, ice=None):
        """
    ...
    def compute_sedflux(self, Acell, elev, qpr, nbe, rain, fillH, dt, actlay, rockCk, rivqs,
        ...
        newdt  = numpy.copy(dt)
        q_prop = numpy.copy(qpr)
        sed_be = numpy.copy(nbe)
        ...
        if self.spl:
            ...
            cdepo, cero, sedload, slopeTIN, flowdensity,n_at_be,qdep = flowalgo.streampower(self.critdens, self.localstack,self.receivers,self.pitID, \
                     self.pitVolume,self.pitDrain,self.xycoords,Acell,self.maxh,self.maxdep,self.discharge,self.a,self.b,fillH, \
                     elev,rivqs,eroCoeff,actlay,perc_dep,slp_cr,sealevel,sealevel+self.deepb,newdt,self.borders,nbe,qpr)
            if self.depo == 0:
                volChange = cero
            else:
                volChange = cdepo+cero
            if verbose:
                print("   - Compute sediment volumetric flux ", time.time() - time1)
                time1 = time.time()
            ...
            if verbose:
                print("   - Compute depressions connectivity ", time.time() - time1)
                time1 = time.time()
            ...
            if newdt < dt:
                ...
                volChange = cdepo+cero
                if verbose:
                    print("   - Compute volumetric fluxes with updated dt ", time.time() - time1)
                    time1 = time.time()
            ...
            if verbose:
                print("   - Compute erosion ", time.time() - time1)
                time1 = time.time()
            ...
            if self.depo == 0:
                ...
                if nsea > 0:
                    ...
                    depo[seaIDs,:] = 0.
                    if verbose:
                        print("   - Compute marine deposition ", time.time() - time1)
                        time1 = time.time()
        ...
        return newdt, sedflux, erosion, deposition, sed_be, q_prop, slopeTIN
