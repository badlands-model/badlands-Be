...
class Model(object):
    ...
    **ld** the longshore drift, **sl** the sea-level, **u** the tectonic, **f** the flexural isostasy and **r** the rainfall patterns.
    ...
    def _build_mesh(self, filename, verbose):
        ...
        if self.input.Qmap :
            ...
            if not self.input.restart:
                self.qprop[self.inIDs] = self.force.get_Qtz(self.inIDs)
            else:
                self.qprop = self.force.reload_Qtz()
        ...
        self.hillslope.icediff = self.input.ice_diff
    ...
    def run_to_time(self, tEnd, verbose=False):
        ...
        # Compute initial 10Be concentration in bedrock (CP)
        if self.input.Qmap is not None and self.input.initdenrate > 0.:
            ...
            self.ice_thck = np.zeros((self.totPts))
            self.topo_shield = self.cosmo.topography_shielding(self.input.shieldfact,self.ice_thck,
                    self.force.sealevel,self.elevation,self.FVmesh.neighbours,self.FVmesh.edge_length)
            self.bedrock_Be,self.prodrate = self.cosmo.compute_init_Be(self.force.sealevel,self.elevation,
                    self.topo_shield,self.qprop,self.input.initdenrate)

        # Perform main simulation loop
        while self.tNow < tEnd:
            
            # uncomment to create a 10Be peak as sediment tracer
            #if self.tNow == 30000.:
            #    bedrock_Be_sauv = np.copy(self.bedrock_Be)
            #if self.tNow > 30000. and self.tNow < 35000.:
            #    listpts = np.where(np.logical_and(self.elevation > 1800, self.qprop > 0))[0]
            #    self.bedrock_Be[listpts] = 2.e6
            #if self.tNow == 35000.:
            #   self.bedrock_Be[listpts] = bedrock_Be_sauv[listpts]

            # At most, display output every 5 seconds
            tloop = time.time() - last_time
            if time.time() - last_output >= 5.0:
            ...
            if self.tNow >= self.force.next_flexure:
                ...
                self.tinFlex = self.flex.get_flexure(self.elevation, self.cumdiff,
                            self.force.sealevel,self.recGrid.boundsPt, self.ice_thck, initFlex=False)
                # Get border values
                ...
                print("   - Compute flexural isostasy %0.02f seconds" % (time.time() - flextime))
            ...
            if self.tNow >= self.force.next_wave:
                ...
                print("   - Compute wave-induced sediment transport %0.02f seconds" % (time.time() - wavetime))
            ...
            if self.tNow >= self.next_carbStep:
                ...
                print("   - Compute carbonate growth %0.02f seconds" % (time.time() - carbtime))
            ...
            self.fillH, self.elevation = buildFlux.streamflow(self.input, self.FVmesh, self.recGrid, self.force, self.hillslope, \
                                              self.flow, self.elevation, self.lGIDs, self.rain, self.tNow, self.melt_water, self.ice_thck, verbose)
            ...
            if self.tNow >= self.force.next_display:
                ...
                checkPoints.write_checkpoints(self.input, self.recGrid, self.lGIDs, self.inIDs, self.tNow,
                                            ...
                                            self.outputStep, self.prop, self.mapero, self.cumflex,self.topo_shield,self.bedrock_Be,
                                            self.qprop,self.prodrate,self.ice_thck)
                ...
                if self.straTIN is not None and self.outputStep % self.input.tmesh==0:
                    ...
                    print("   - Write sediment mesh output %0.02f seconds" % (time.time() - meshtime))
                ...
                if self.carbTIN is not None and self.outputStep % self.input.tmesh==0:
                    ...
                    print("   - Write carbonate mesh output %0.02f seconds" % (time.time() - meshtime))
        ...
        if self.input.udw == 0 or self.tNow == self.input.tEnd or self.tNow == self.force.next_display:
            checkPoints.write_checkpoints(self.input, self.recGrid, self.lGIDs, self.inIDs, self.tNow, \
                                self.FVmesh, self.tMesh, self.force, self.flow, self.rain, \
                                self.elevation, self.fillH, self.cumdiff, self.cumhill, self.cumfail, self.wavediff, \
                                self.outputStep, self.prop, self.mapero, self.cumflex,self.topo_shield,\
                                self.bedrock_Be,self.qprop,self.prodrate,self.ice_thck)
