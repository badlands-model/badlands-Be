...
def write_hdf5(folder, h5file, step, coords, elevation, rain, discharge, cumdiff, cumhill, cumfail,
               topshield, nbe, qprop, prate, ice, cells, rainOn, eroOn, erodibility, area, waveOn, beOn, 
               iceOn, waveH, waveS, wavediff, rockOn, prop, sealevel):
    """
    ...
    with h5py.File(h5file, "w") as f:
        ...
        if iceOn:
            f.create_dataset('ice',shape=(len(discharge), 1), dtype='float64', compression='gzip')
            f["ice"][:,0] = ice

        if beOn:
            f.create_dataset('topshield',shape=(len(discharge), 1), dtype='float64', compression='gzip')
            f["topshield"][:,0] = topshield
            f.create_dataset('prate',shape=(len(discharge), 1), dtype='float64', compression='gzip')
            f["prate"][:,0] = prate
            f.create_dataset('nbe',shape=(len(discharge), 1), dtype='float64', compression='gzip')
            f["nbe"][:,0] = nbe
            f.create_dataset('qprop',shape=(len(discharge), 1), dtype='float64', compression='gzip')
            f["qprop"][:,0] = qprop
...
def write_hdf5_flexure(folder, h5file, step, coords, elevation, rain, discharge, cumdiff, cumhill, cumfail,
                       cumflex, topshield, nbe, qprop, prate, ice, cells, rainOn, eroOn, erodibility, area, waveOn, 
                       beOn, iceOn, waveH,  waveS, wavediff, rockOn, prop, sealevel):
    """
    This function writes for each processor the **hdf5** file containing surface information
...
def write_xmf(folder, xmffile, xdmffile, step, t, elems, nodes, h5file, sealevel,
              flexOn, rainOn, eroOn, waveOn, rockOn, beOn, iceOn, nbSed):
    """
    ...
    if iceOn:
        f.write('         <Attribute Type="Scalar" Center="Node" Name="Ice">\n')
        f.write('          <DataItem Format="HDF" NumberType="Float" Precision="4" ')
        f.write('Dimensions="%d 1">%s:/ice</DataItem>\n'%(nodes[p],pfile))
        f.write('         </Attribute>\n')

    if beOn:
        f.write('         <Attribute Type="Scalar" Center="Node" Name="Topo_shielding">\n')
        f.write('          <DataItem Format="HDF" NumberType="Float" Precision="4" ')
        f.write('Dimensions="%d 1">%s:/topshield</DataItem>\n'%(nodes[p],pfile))
        f.write('         </Attribute>\n')
        f.write('         <Attribute Type="Scalar" Center="Node" Name="Production_rate">\n')
        f.write('          <DataItem Format="HDF" NumberType="Float" Precision="4" ')
        f.write('Dimensions="%d 1">%s:/prate</DataItem>\n'%(nodes[p],pfile))
        f.write('         </Attribute>\n')
        f.write('         <Attribute Type="Scalar" Center="Node" Name="Be_concentration">\n')
        f.write('          <DataItem Format="HDF" NumberType="Float" Precision="4" ')
        f.write('Dimensions="%d 1">%s:/nbe</DataItem>\n'%(nodes[p],pfile))
        f.write('         </Attribute>\n')
        f.write('         <Attribute Type="Scalar" Center="Node" Name="Quartz_map">\n')
        f.write('          <DataItem Format="HDF" NumberType="Float" Precision="4" ')
        f.write('Dimensions="%d 1">%s:/qprop</DataItem>\n'%(nodes[p],pfile))
        f.write('         </Attribute>\n')
