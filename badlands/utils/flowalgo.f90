...
subroutine diffmarine(pyZ, pyQpr, pyNBe, pyBord, pyDepoH, pyNgbs, pyEdge, pyDist, pyCoeff, pyGIDs, &
                  slvl, pymaxth, tstep, pyDiff, new_qprop, new_Be, mindt, pylocalNb, pyglobalNb)
  ...
  real(kind=8),dimension(pyglobalNb),intent(in) :: pyQpr
  real(kind=8),dimension(pyglobalNb),intent(in) :: pyNBe
  ...
  real(kind=8) :: pyDiffQz, pyDiffBe, pyDiffpos,rhosed
  real(kind=8),intent(out) :: mindt
  real(kind=8),dimension(pyglobalNb),intent(out) :: pyDiff
  real(kind=8),dimension(pyglobalNb),intent(out) :: new_qprop
  real(kind=8),dimension(pyglobalNb),intent(out) :: new_Be
  ...
  do k = 1, pylocalNb
      ...
      loop: do p =1,20
        ...
        if(pyBord(ngbid)>0.)then
          ...
          elseif(pyDepoH(ngbid)>pymaxth .and. pyZ(gid)<pyZ(ngbid) .and. pyZ(ngbid)<slvl)then
            pyDiff(gid) = pyDiff(gid) + pyCoeff(gid)*flx
            pyDiffpos = pyDiffpos + pyCoeff(gid)*flx
            pyDiffQz = pyDiffQz + pyCoeff(gid)*flx*pyQpr(ngbid)
            pyDiffBe = pyDiffBe + pyCoeff(gid)*flx*pyQpr(ngbid)*pyNBe(ngbid)*1000*rhosed
          endif
    ...
    if (pyDiffpos > 0. .and. pyDiffQz > 0.) then
            new_qprop(gid) = pyDiffQz/pyDiffpos
...
subroutine difffailure(pyZ, pyQpr, pyBe, pyBord, pyDepoH, pyNgbs, pyEdge, pyDist, pyCoeff, pyGIDs, &
                  pymaxth, tstep, pyDiff, new_qprop, new_Be, mindt, pylocalNb, pyglobalNb)
  ...
  real(kind=8),dimension(pyglobalNb),intent(in) :: pyZ
  real(kind=8),dimension(pyglobalNb),intent(in) :: pyQpr
  real(kind=8),dimension(pyglobalNb),intent(in) :: pyBe
  real(kind=8),dimension(pyglobalNb),intent(in) :: pyCoeff
  real(kind=8),dimension(pyglobalNb),intent(in) :: pyDepoH
  real(kind=8),dimension(pyglobalNb,20),intent(in) :: pyEdge
  real(kind=8),dimension(pyglobalNb,20),intent(in) :: pyDist
  ...
  integer :: k, gid, ngbid, p
  real(kind=8) :: flx,rhosed,pyDiffQz,pyDiffBe,sumDiffpos
  ...
  do k = 1, pylocalNb
      ...
      loop: do p =1,20
        ...
        if(pyBord(ngbid)>0.)then
          ...
          elseif(pyDepoH(ngbid)>pymaxth .and. pyZ(gid)<pyZ(ngbid))then
            pyDiff(gid) = pyDiff(gid) + pyCoeff(gid)*flx
            pyDiffQz = pyDiffQz + pyCoeff(gid)*flx*pyQpr(gid)
            pyDiffBe = pyDiffBe + pyCoeff(gid)*flx*pyQpr(gid)*rhosed*1000
            sumDiffpos = sumDiffpos + pyCoeff(gid)*flx
          endif
        elseif(pyBord(ngbid)<1)then
          if(pyDepoH(gid)>pymaxth .and. pyZ(gid)>pyZ(ngbid))then
            flx = pyEdge(gid,p)*(pyZ(ngbid)-pyZ(gid))/pyDist(gid,p)
    ...
    if (sumDiffpos > 0. .and. pyDiffQz > 0.) then
        new_qprop(gid) = pyDiffQz / sumDiffpos
        new_Be(gid) = pyDiffBe / pyDiffQz / rhosed / 1000.
    endif
  enddo
...
subroutine diffsedmarine(pyZ, pyQpr, pyBe, pyBord, pyDepo, pyDepoH, slvl, pymaxth, pyCoeff, pyNgbs, pyEdge, &
                    pyDist, pyGIDs, pyDiff, sumDiff, new_qprop, new_Be, pylocalNb, pyglobalNb, pyRockNb)
  ...
  real(kind=8) :: pyDiffQz, pyDiffBe, sumDiffpos, rhosed
  ...
  do k = 1, pylocalNb
        ...
        if(pyBord(ngbid)>0.)then
          ...
          elseif(pyDepoH(ngbid)>pymaxth .and. pyZ(gid)<pyZ(ngbid) .and. pyZ(ngbid)<slvl)then
            ...
            if(sfrac>0.)then
              ...
              pyDiff(gid,:) = pyDiff(gid,:) + tfrac*sed(:)
              sumDiff(gid) = sumDiff(gid) + tfrac*tsed
              if (tfrac * tsed > 0) then
                      sumDiffpos = sumDiffpos + tfrac * tsed
                      pyDiffQz = pyDiffQz + tfrac*tsed*pyQpr(ngbid)
                      pyDiffBe = pyDiffBe + tfrac*tsed*pyQpr(ngbid)*pyBe(ngbid)*1000*rhosed
              endif
            endif
          endif
        elseif(pyBord(ngbid)<1)then
    ...
    if (sumDiffpos > 0 .and. pyDiffQz > 0) then
        new_qprop(ngbid) = pyDiffQz / sumDiffpos
        new_Be(ngbid) = pyDiffBe / pyDiffQz / rhosed / 1000.
    endif
  enddo
...
subroutine diffsedhillslope(pyZ, pyQpr, pyBe, pyBord, difflay, maxlayh, pyCoeff, pyNgbs, pyEdge, pyDist, pyGIDs,  &
                     sumDiff, new_qprop, new_Be, ero, depo, pylocalNb, pyglobalNb, pyRockNb)
  ...
  integer :: k, gid, ngbid, p, r
  real(kind=8) :: flx, frac, sfrac, tfrac, sed(pyRockNb), tsed, rhosed
  real(kind=8) :: pyDiffQz, pyDiffBe, sumDiffpos
      ...
      if(pyBord(ngbid)>0.)then
        ...
        elseif(pyZ(gid)<pyZ(ngbid))then
          ...
          if(sfrac>0.)then
            ...
            if(tsed<0.) ero(gid,:) =  ero(gid,:) + tfrac*sed(:)
            if(tsed>0.) then
                    depo(gid,:) =  depo(gid,:) + tfrac*sed(:)
                    sumDiffpos = sumDiffpos + tfrac * tsed
                    pyDiffQz = pyDiffQz + tfrac * tsed * pyQpr(ngbid)
                    pyDiffBe = pyDiffBe + tfrac * tsed * pyQpr(ngbid) * pyBe(ngbid) * rhosed * 1000.
            endif
            sumDiff(gid) = sumDiff(gid) + tfrac*tsed
          endif
        endif
...
pyMaxD, pyDischarge, pyA, pyB, pyFillH, pyElev, pyRiv, Cero, actlay, perc_dep, slp_cr, sea, db, dt, &
borders, pyNBe, pyQprop, pyDepo, pyEro, sedFluxes, slope, pyDensity, pyDepoBe, pyDepoQ, &
pylNodesNb, pygNodesNb, pyRockNb)
  ...
  real(kind=8),dimension(pygNodesNb),intent(in) :: pyElev
  real(kind=8),dimension(pygNodesNb),intent(in) :: pyNBe
  real(kind=8),dimension(pygNodesNb),intent(in) :: pyQprop
  ...
  real(kind=8),dimension(pyRockNb) :: SPL, Qs, Qb, frck, erodep, pitDep, fracQ, fracBe
  real(kind=8),dimension(pyRockNb) :: Q_Qz, Q_Be, erodepQ, erodepBe, pitDepQ, pitDepBe 
  real(kind=8),dimension(pygNodesNb) :: upZ, updist, pitVol,hypyc
  real(kind=8),dimension(pygNodesNb,pyRockNb) :: bedFluxes
  real(kind=8),dimension(pygNodesNb,pyRockNb) :: Qz_Fluxes
  real(kind=8),dimension(pygNodesNb,pyRockNb) :: Be_Fluxes
  ...
  do n = pylNodesNb, 1, -1
    ...
    if(totspl < 0.)then
      ...
      if(pyRockNb>1)then
        do r = 1, pyRockNb
          ...
          erodepQ(r) = erodep(r) * pyQprop(donor)
          erodepBe(r) = erodepQ(r) * pyNBe(donor) * rhosed * 1000
          Qs(r) = -erodep(r) + sedFluxes(donor,r)
          Q_Qz(r) = -erodepQ(r) + Qz_Fluxes(donor,r)
          Q_Be(r) = -erodepBe(r) + Be_Fluxes(donor,r)
          if(bedslptype > 0) Qb(r) = -erodep(r)*bedfrac + bedFluxes(donor,r)
        enddo
      else
        erodep(1) = SPL(1) * dt * pyArea(donor)
        erodepQ(1) = erodep(1) * pyQprop(donor)
        erodepBe(1) = erodepQ(1) * pyNBe(donor) * rhosed * 1000
        Qs(1) = -erodep(1) + sedFluxes(donor,1)
        Q_Qz(1) = -erodepQ(1) + Qz_Fluxes(donor,1)
        Q_Be(1) = -erodepBe(1) + Be_Fluxes(donor,1)
    ...
    elseif( totspl >= 0. .and. pyArea(donor) > 0.)then
      ...
      elseif(pyElev(donor) <= sea .and. hypyc(donor) <= 0.)then
        ...
        do r = 1, pyRockNb
          ...
          erodepQ(r) = Qz_Fluxes(donor,r)
          erodepBe(r) = Be_Fluxes(donor,r)
        enddo
        Qs = 0.
        Q_Qz = 0.
        Q_Be = 0.
      ...
      elseif(maxh > 0. .and. waterH == 0. .and. donor /= recvr )then
        ...
        if(totflx/pyArea(donor) < maxh)then
          ...
          do r = 1, pyRockNb
            ...
            erodepBe(r) = erodep(r)*fracBe(r)
            Qs(r) = sedFluxes(donor,r) - erodep(r)
            Q_Qz(r) = Qz_Fluxes(donor,r) - erodepQ(r)
            Q_Be(r) = Be_Fluxes(donor,r) - erodepBe(r)
      ...
      elseif(donor == recvr .and. pyArea(donor) > 0.)then
        ...
        do r = 1, pyRockNb
          ...
          Q_Qz(r) = Qz_Fluxes(donor,r)
          Q_Be(r) = Be_Fluxes(donor,r)
    ...
    if(maxval(pitDep)==0.)then
      do r = 1, pyRockNb
        ...
        Qz_Fluxes(recvr,r) = Qz_Fluxes(recvr,r) + Q_Qz(r)
        Be_Fluxes(recvr,r) = Be_Fluxes(recvr,r) + Q_Be(r)
        ...
        if(erodep(r)<0.)then
          ...
          pyDepoQ(donor) = pyDepoQ(donor) + erodepQ(r)
          pyDepoBe(donor) = pyDepoBe(donor) + erodepBe(r)
        endif
      enddo
      ...
      do while(totdist > 0.)
        ...
        if(pyfillH(tmpID)<sea)then
          if(pyElev(donor)<sea)then
            do r = 1, pyRockNb
              ...
              pyDepoQ(donor) = pyDepoQ(donor) + pitDepQ(r)
              pyDepoBe(donor) = pyDepoBe(donor) + pitDepBe(r)
            enddo
            totdist = 0.
          else
            do r = 1, pyRockNb
              sedFluxes(recvr,r) = sedFluxes(recvr,r) + pitDep(r)
              Qz_Fluxes(recvr,r) = Qz_Fluxes(recvr,r) + pitDepQ(r)
              Be_Fluxes(recvr,r) = Be_Fluxes(recvr,r) + pitDepBe(r)
            enddo
            totdist = 0.
          endif
          nID = recvr
        ...
        elseif(tmpdist+totdist<=pitVol(tmpID))then
          do r = 1, pyRockNb
            ...
            pyDepoQ(tmpID) = pyDepoQ(tmpID) + pitDepQ(r)
            pyDepoBe(tmpID) = pyDepoBe(tmpID) + pitDepBe(r)
          enddo
          totdist = 0.
          nID = tmpID
