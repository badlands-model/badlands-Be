...
subroutine marine_distribution(elevation, seavol, qpr, nbe, sealevel, border, depIDs, pySlp, &
           diffsed, diffQz, diffBe, pydnodes, pyIDs, pyRockNb)
  ...
  real(kind=8),dimension(pydnodes),intent(in) :: qpr, nbe
  real(kind=8),dimension(pydnodes),intent(in) :: pySlp

  real(kind=8),dimension(pydnodes,pyRockNb),intent(out) :: diffsed
  real(kind=8),dimension(pydnodes),intent(out) :: diffQz, diffBe
  real(kind=8),dimension(pydnodes) :: elev, seadep, seavoltot, rockdeporatio
  real(kind=8),dimension(pydnodes) :: newelev, totdep_Q, totdep_Be, totdep, qdep, bedep

  integer :: it, s, m, n, p, k, pid, nid, id, nup, ndown, ngbh(20)
  real(kind=8) :: dh, vol, minz, maxz, dprop, rhosed, ratiodepo
  ...
  do s = 1, pyRockNb
    do k = 1, pyIDs
      seavoltot(k) = seavoltot(k) + seavol(k,s)
    enddo
  enddo
  do s = 1, pyRockNb
    do k = 1, pyIDs
        if (seavoltot(k) > 0) then
            rockdeporatio(k) = seavol(k,s) / seavoltot(k)
        endif
    enddo
  enddo
  ...
  do s = 1, pyRockNb
    ...
    do m = 1, diffnbmax
      ...
      bedep  = nbe/float(diffnbmax)*rockdeporatio
      qdep   = qpr/float(diffnbmax)*rockdeporatio      
      do k = 1, pyIDs
        n = depIDs(k)+1
        id = n
        pid = n
        ...
        sfd_loop: do
            ...
            if(it>max_it_cyc)then
              ...
              totdep(id)     = totdep(id) + seadep(id)/area(id)
              totdep_Q(id)   = totdep_Q(id)  + qdep(id)/area(id)
              totdep_Be(id)  = totdep_Be(id) + bedep(id)/area(id)
              seadep(id) = 0.
              qdep(id)  = 0.
              bedep(id) = 0.
              exit sfd_loop
            ...
            if(seadep(id)<vol)then
              ...
              ratiodepo  = vol/seadep(id)
              seadep(id) = seadep(id) - vol
              elev(id)   = elev(id) + vol/area(id)
              totdep(id) = totdep(id) + vol/area(id)
              totdep_Q(id)   = totdep_Q(id) + ratiodepo * qdep(id)/area(id)
              totdep_Be(id)  =  totdep_Be(id) + ratiodepo * bedep(id)/area(id)
              qdep(id)  = qdep(id) - ratiodepo * qdep(id)
              bedep(id) = bedep(id) - ratiodepo * bedep(id)
            endif
            if(seadep(id)>0.)then
              minz = elev(id)
              nid = 0
              ...
              if(nid==0)then
                ...
                if(seadep(id) > dh*area(id))then
                  ratiodepo = dh*area(id)/seadep(id)
                  elev(id)   = elev(id) + dh
                  totdep_Q(id)   = totdep_Q(id)  + ratiodepo * qdep(id)/area(id)
                  totdep_Be(id)  = totdep_Be(id) + ratiodepo * bedep(id)/area(id)
                  totdep(id)     = totdep(id) + dh
                  ...
                  qdep(nid)    = qdep(nid)   + qdep(id)
                  bedep(nid)   = bedep(nid)  + bedep(id)
                  seadep(id) = 0.
                  qdep(id)  = 0.
                  bedep(id) = 0.
                  id = nid
